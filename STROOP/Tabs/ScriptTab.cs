...
{
    public partial class ScriptTab : STROOPTab
    {
        ...
        public override void InitializeTab()
        {
            ...
            checkBoxScriptRunContinuously.Click += (sender, e) =>
            {
                if (checkBoxScriptRunContinuously.Checked)
                {
                    _script.SetScript(richTextBoxScript.Text);
                }
                _script.SetIsEnabled(checkBoxScriptRunContinuously.Checked);
                richTextBoxScript.ReadOnly = checkBoxScriptRunContinuously.Checked;
            };

            buttonScriptRunOnce.Click += (sender, e) =>
            {
                _script.SetScript(richTextBoxScript.Text);
                _script.Run();
            };

            buttonScriptInstructions.Click += (sender, e) =>
            {
                InfoForm.ShowValue(
                    string.Join("\r\n", _instructions),
                    "Instructions",
            ...
            buttonScriptExamples.ContextMenuStrip = new ContextMenuStrip();
            for (int i = 0; i < _exampleNames.Count; i++)
            {
                ToolStripMenuItem item = new ToolStripMenuItem(_exampleNames[i]);
                string text = string.Join("\r\n", _exampleLines[i]);
                item.Click += (sender, e) => richTextBoxScript.Text = text;
                buttonScriptExamples.ContextMenuStrip.Items.Add(item);
            }
            buttonScriptExamples.Click += (sender, e) =>
                buttonScriptExamples.ContextMenuStrip.Show(Cursor.Position);
        }
