...
namespace STROOP.Tabs.MapTab.Simulations
{
    public class CorkBox
    {
        ...
        public CollisionStructure WallTris, FloorTris;

        public CorkBox(float x, float y, float z, CollisionStructure wallTris, CollisionStructure floorTris)
        {
            X = x;
            Y = y;
            Z = z;
        ...
        public override string ToString() => $"pos=({X},{Y},{Z}) spd=({XSpeed},{YSpeed},{ZSpeed},{HSpeed}) yaw={Yaw} itimer={InactivityTimer} dead={Dead}";
        ...
        private short object_step()
        {
            ...
            (TriangleDataModel staticFloor, float floorY) = FloorTris.FindFloorAndY(objX + objVelX, objY, objZ + objVelZ);
            StaticFloor = staticFloor;

            if (turn_obj_away_from_steep_floor(StaticFloor, floorY, objVelX, objVelZ) == 1)
            {
                waterY = CollisionUtilities.GetWaterAtPos(objX + objVelX, objZ + objVelZ);
                if (waterY > objY)
                {
        ...
        {
            int numCollisions = WallTris.GetNumWallCollisions(
                objNewX, objY, objNewZ, 60, 50);
