...
namespace STROOP.Tabs.MapTab.Simulations
{
    public class CollisionStructure
    {
        ...
        List<TriangleDataModel>[,] cells = new List<TriangleDataModel>[NUM_CELLS, NUM_CELLS];
         ...
         */
        void add_surface(TriangleDataModel tri)
        {
            ...
            for (cellZ = minCellZ; cellZ <= maxCellZ; cellZ++)
                for (cellX = minCellX; cellX <= maxCellX; cellX++)
                    cells[cellX, cellZ].Add(tri);
        }

        public CollisionStructure(List<TriangleDataModel> tris)
        {
            for (int y = 0; y < NUM_CELLS; y++)
                for (int x = 0; x < NUM_CELLS; x++)
                    cells[x, y] = new List<TriangleDataModel>();

            foreach (var tri in tris)
                add_surface(tri);
        }

        public List<TriangleDataModel> GetNearbyTriangles(float x, float z, float size)
        {
        ...
        {
            var tris = GetNearbyTriangles(x, z, 0);
            foreach (var tri in tris)
            {
        ...
        public int GetNumWallCollisions(
            float marioX, float marioY, float marioZ, float radius, float offsetY)
        {
            var surfs = GetNearbyTriangles(marioX, marioZ, radius);
